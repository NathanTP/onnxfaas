...
def read_squad_examples(input_file=None, input_str=None):
    """Read a SQuAD json file into a list of SquadExample."""
    if input_file is not None:
        with open(input_file, "r") as f:
            input_data = json.load(f)["data"]
    elif input_str is not None:
        input_data = json.loads(input_str)["data"]
    else:
        raise RuntimeError("Either input_file or input_str is required")
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    output_pred = json.dumps(all_predictions, indent=4)
    output_nbest = json.dumps(all_nbest_json, indent=4)
    ...
    return (output_pred, output_nbest)
